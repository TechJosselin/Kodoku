...
**Kodoku** est un jeu s&box (survie/exploration coop) actuellement **reconstruit presque depuis zéro**. `Editor/` reste réduit à des `global using`. `Code/` contient plusieurs systèmes validés par test réel host/client : `KodokuPlayerComponent` (identification du pawn local/proxy), `PlayerVitalsComponent` (santé/endurance/faim/soif/radiation, host-authoritative), un HUD local minimal (`Code/UI/Hud/GameHud.razor`), une fondation d'items — `ItemDefinition`, `ItemInstance`, `WorldItemComponent`, `LootSpawnPointComponent` —, un noyau local d'inventaire à grille `InventoryContainer` (validé Tests A à O, non networké), et désormais `PlayerInventoryComponent` + `WorldItemPickupComponent` (pickup interactif host-authoritative avec snapshot d'inventaire répliqué au seul propriétaire, validés par test réel host/client — voir [docs/architecture/ITEM_ARCHITECTURE.md](docs/architecture/ITEM_ARCHITECTURE.md)). Le système natif d'inventaire s&box a été évalué et n'a pas été retenu — voir [ADR-0005](docs/decisions/ADR-0005-CUSTOM-INVENTORY.md). Ne sont **toujours pas implémentés** : drop, transfert entre joueurs, équipement, UI d'inventaire (au-delà d'un panneau debug), stack merge/split, mort/respawn, sauvegarde. Voir [docs/status/CURRENT_STATE.md](docs/status/CURRENT_STATE.md) pour l'état factuel à jour.
