...
**Kodoku** est un jeu s&box (survie/exploration coop) actuellement **reconstruit presque depuis zéro**. `Editor/` reste réduit à des `global using`. `Code/` contient plusieurs systèmes validés par test réel host/client : `KodokuPlayerComponent` (identification du pawn local/proxy), `PlayerVitalsComponent` (santé/endurance/faim/soif/radiation, host-authoritative), un HUD local minimal (`Code/UI/Hud/GameHud.razor`), et une première fondation d'items — `ItemDefinition` (ressource `.item`), `ItemInstance` (état runtime), `WorldItemComponent` (représentation monde, réplication réseau host-authoritative minimale) et `LootSpawnPointComponent` (génération de loot host-authoritative, V1 mono-item) — voir [docs/architecture/ITEM_ARCHITECTURE.md](docs/architecture/ITEM_ARCHITECTURE.md). Ne sont **toujours pas implémentés** : inventaire joueur, pickup interactif, conteneurs, équipement, consommation, armes, persistance, mort/respawn. Voir [docs/status/CURRENT_STATE.md](docs/status/CURRENT_STATE.md) pour l'état factuel à jour.
