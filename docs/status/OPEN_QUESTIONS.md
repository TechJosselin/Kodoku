...
- **Drop réseau** — **implémenté le 2026-07-15** (`PlayerItemDropComponent`), non encore validé par test réel — voir [ITEM_ARCHITECTURE.md](../architecture/ITEM_ARCHITECTURE.md), section « Drop d'item — V1 ». Une question technique soulevée pendant cette implémentation reste sans réponse confirmée par le moteur : aucun précédent trouvé (code moteur inspecté ni ce projet) sur l'inclusion ou non d'une propriété `[Sync]` définie *avant* `NetworkSpawn()` dans l'état initial transmis aux autres clients — contournée par construction (publication réseau toujours après le spawn), pas résolue.
- **Transfert entre conteneurs/joueurs, stack merge/split, inventaires imbriqués, équipement par emplacement corporel, persistance/sauvegarde de l'inventaire** — tous non implémentés, hors périmètre du noyau `InventoryContainer` actuel.
