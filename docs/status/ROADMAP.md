...
- **Conteneurs du monde (regroupement de plusieurs items déposés dans une seule boîte, ex. `Cardboard_box`) : non commencés.** La condition d'entrée posée le 2026-07-15 (les transactions atomiques d'équipement/déséquipement et la synchronisation de l'inventaire propriétaire doivent être validées avant d'introduire un conteneur public partagé, étape 8) est désormais **remplie** depuis le 2026-07-18 (équipement et utilisation d'item validés par test réel — voir étape 8 ci-dessus), mais aucune conception ni implémentation de conteneur du monde n'a encore commencé — voir [../architecture/ITEM_ARCHITECTURE.md](../architecture/ITEM_ARCHITECTURE.md) et [OPEN_QUESTIONS.md](OPEN_QUESTIONS.md). Cette étape 9 n'est donc **pas** terminée dans son ensemble malgré son statut « fait pour son périmètre » : le ramassage/dépôt individuel est validé pour son périmètre core, les conteneurs du monde restent un sous-objectif non commencé de l'« Objectif » ci-dessus.
