...
4. Stacks, split/merge et quantités (étape 8) — audit de conception terminé (jalon 0). **Jalon 1 (Stack Core pur) implémenté et validé localement** (65 scénarios runtime PASS, exécution réelle en éditeur, 2026-07-23, corrigé après relecture technique et revalidé le 2026-07-24 — `InventoryContainer.TryGrowQuantity`, `InventoryStackTransactions`/`StackTransactionResult`/`StackTransactionFailureReason`, aucun réseau ni UI) — voir [../architecture/INVENTORY_STACK_ARCHITECTURE.md](../architecture/INVENTORY_STACK_ARCHITECTURE.md) pour le détail. Branche `feature/inventory-stacks-core` : push et Pull Request encore à faire. Jalons 2 à 5 (réseau joueur, transferts conteneur, drop partiel, auto-merge) **non commencés**.
