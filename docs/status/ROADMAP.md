...
## Ordre de priorité actuel (révisé le 2026-07-18)
...
7. **Spike réseau multi-viewer** (étape 9) — prérequis technique bloquant, distinct de l'implémentation elle-même : confirmer le mécanisme de diffusion réseau ciblée à plusieurs destinataires (`Rpc.FilterInclude`/`FilterExclude` ou équivalent), aucun précédent testé dans ce projet. Voir [../architecture/WORLD_CONTAINER_ARCHITECTURE.md](../architecture/WORLD_CONTAINER_ARCHITECTURE.md), section 7 et Spike S0.
8. **Implémentation World Containers V1** (étape 9) — **seulement après validation du spike ci-dessus**, jamais en parallèle ni avant. Conteneurs du monde et regroupement des drops.
9. Scènes, zones et extraction (étape 11).
10. Persistance et reconnexion (étape 12).
...
- **Conteneurs du monde (regroupement de plusieurs items déposés dans une seule boîte, ex. `Cardboard_box`) : conception en cours, non implémentés.** La condition d'entrée posée le 2026-07-15 (les transactions atomiques d'équipement/déséquipement et la synchronisation de l'inventaire propriétaire doivent être validées avant d'introduire un conteneur public partagé, étape 8) est **remplie** depuis le 2026-07-18 (équipement et utilisation d'item validés par test réel — voir étape 8 ci-dessus). Une mission de conception dédiée (branche `design/world-containers`, non mergée) a produit une architecture complète — voir [../architecture/WORLD_CONTAINER_ARCHITECTURE.md](../architecture/WORLD_CONTAINER_ARCHITECTURE.md) : `WorldContainerComponent` autonome (pas de réutilisation de `PlayerInventoryComponent`), transferts atomiques par ordre des mutations (sans rollback dédié, même patron que l'équipement — comportement vérifié par audit direct de `InventoryContainer.TryAddFirstFit`, pas supposé), matrice de tests A à R préparée mais non exécutée. **Aucun code de production n'existe.**
  - **Cette étape se découpe désormais en deux jalons distincts, non parallélisables** :
    1. **Spike réseau multi-viewer** — prérequis technique bloquant. Le mécanisme réseau exact permettant de diffuser un même contenu à un sous-ensemble variable de connexions (`Rpc.FilterInclude`/`FilterExclude` ou un envoi individuel par `Connection`) est **documenté ou identifié comme disponible, mais son comportement exact n'est pas encore vérifié dans ce projet** — aucun appel de ce type n'existe dans le code Kodoku actuel. Couverture minimale du spike (« Spike S0 », voir [../architecture/WORLD_CONTAINER_ARCHITECTURE.md](../architecture/WORLD_CONTAINER_ARCHITECTURE.md), section 17) : host + deux clients distants si possible ; ajout et retrait de viewers ; snapshot ciblé à un seul viewer ; snapshot identique reçu par deux viewers ; exclusion effective d'un non-viewer ; fermeture/invalidation ciblée d'un viewer ; nettoyage après déconnexion sans exception ni référence persistante. **Ce spike ne valide pas les conteneurs eux-mêmes**, seulement le transport.
    2. **Implémentation World Containers V1** — commence **seulement après** que le spike ci-dessus a produit un résultat concluant (option confirmée, section 7 du document mise à jour en conséquence). Aucune implémentation de conteneur ne doit précéder cette validation.
  - Voir [OPEN_QUESTIONS.md](OPEN_QUESTIONS.md) pour le détail des questions encore ouvertes. Cette étape 9 n'est donc **pas** terminée dans son ensemble malgré son statut « fait pour son périmètre » : le ramassage/dépôt individuel est validé pour son périmètre core, les conteneurs du monde restent un sous-objectif conçu mais non implémenté de l'« Objectif » ci-dessus — **ni implémentés, ni validés**.
