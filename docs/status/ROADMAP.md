...
2. Équipement corporel minimal — **conçu, non implémenté** (architecture complète tranchée, voir [../architecture/ITEM_ARCHITECTURE.md](../architecture/ITEM_ARCHITECTURE.md), étape 8).
...
- **Équipement corporel minimal : conçu, non implémenté (mission de conception du 2026-07-16, branche `design/minimal-equipment`).** Le transfert direct d'item entre deux joueurs ne fait pas partie du gameplay prévu de Kodoku et n'est **pas** implémenté. Architecture complète tranchée avant toute écriture de code — voir [../architecture/ITEM_ARCHITECTURE.md](../architecture/ITEM_ARCHITECTURE.md), section « Équipement corporel minimal » : deux slots (`Head`/`Body`), intégré à `PlayerInventoryComponent` (pas de composant séparé), compatibilité par slot unique (`ItemDefinition.EquipmentSlot`), refus propre si slot occupé (aucun swap), snapshot combiné (grille + équipement, une seule révision), équipement privé au propriétaire, atomicité garantie par l'ordre des mutations **sans mécanisme de rollback dédié** (l'étape pouvant échouer s'exécute toujours avant celle qui ne peut pas échouer). Dix scénarios de test préparés (host + client), non exécutés. Prochaine mission : implémentation, puis validation par test réel à deux instances.
