...
4. Stacks, split/merge et quantités (étape 8) — audit de conception terminé (jalon 0). **Jalon 1 (Stack Core pur) implémenté, validé et mergé dans `main`** (65 scénarios runtime PASS, exécution réelle en éditeur, 2026-07-23, corrigé après relecture technique et revalidé le 2026-07-24 — `InventoryContainer.TryGrowQuantity`, `InventoryStackTransactions`/`StackTransactionResult`/`StackTransactionFailureReason`, aucun réseau ni UI ; PR #17 mergée) — voir [../architecture/INVENTORY_STACK_ARCHITECTURE.md](../architecture/INVENTORY_STACK_ARCHITECTURE.md) pour le détail. **Jalon 2 (split et merge réseau joueur) : conception auditée et corrigée, implémentation non commencée** — voir [../architecture/PLAYER_INVENTORY_STACK_NETWORK_ARCHITECTURE.md](../architecture/PLAYER_INVENTORY_STACK_NETWORK_ARCHITECTURE.md) pour l'audit dédié (architecture réseau, signatures RPC, préconditions de fraîcheur, `AppliedRevision`, stratégie `RollbackFailed` à appel unique de `NotifyMutated()`, matrice de tests corrigée à 63 scénarios documentés). Jalons 3 à 5 (transferts conteneur, drop partiel, auto-merge) **non commencés**.
