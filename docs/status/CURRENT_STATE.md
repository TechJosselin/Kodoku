...
- le drop d'item (`PlayerItemDropComponent`) : **validé par test réel pour son périmètre core, y compris l'obstacle devant le joueur et la recherche automatique de position en anneau à 360°** (voir « Mise à jour du 2026-07-15 » ci-dessus et [ITEM_ARCHITECTURE.md](../architecture/ITEM_ARCHITECTURE.md), section « Drop d'item — V1 », sous-section « Position de drop », pour la décision de gameplay finale). Restent non exécutés : quantité conservée, double requête, `InstanceId` invalide, ownership rejeté, rollback.
...
- Conteneurs du monde partagés / regroupement des drops en boîte — non commencés, explicitement reportés (décision du 2026-07-15, condition d'entrée : transfert atomique d'`ItemInstance` entre deux inventaires joueurs) — voir [OPEN_QUESTIONS.md](OPEN_QUESTIONS.md) et [../architecture/ITEM_ARCHITECTURE.md](../architecture/ITEM_ARCHITECTURE.md)
- Équipement, consommation d'objets, transfert entre joueurs (voir « Mise à jour du 2026-07-13 » ci-dessus pour le pickup/inventaire joueur, et « Mise à jour du 2026-07-15 » pour le drop — validé pour son périmètre core, pas absent)
