...
**Statut : trois niveaux distincts, à ne jamais confondre.**
...
- **Whole-item transfers — implémentés et validés par test runtime réel** (branche `feature/world-container-transfers`, à partir de `main`/`dcaff8e`) : `RequestTakeItem`/`RequestStoreItem` sur `WorldContainerComponent`, préflight pur de destination (`InventoryContainer.TryFindFirstFit`), même `ItemInstance`/`InstanceId` déplacée, aucune mutation sur un refus, résultat RPC ciblé non canonique, révisions croisées exactes. **Matrice T0-A à T0-O intégralement validée** (15/15), host + deux clients distants, plusieurs sessions réelles — voir [docs/research/WORLD_CONTAINER_TRANSFER_TESTS.md](../research/WORLD_CONTAINER_TRANSFER_TESTS.md) pour le rapport complet. Détail dans les sections 9 et 17 ci-dessous, mises à jour avec le résultat réel.
- **Déplacement interne ciblé (drag-and-drop) — implémenté et validé par test runtime réel** depuis le 2026-07-21 (`RequestMoveItem`/`RequestStoreItemAt`/`RequestTakeItemAt`, campagne M1, host + client, y compris concurrence à deux fenêtres réellement simultanées) — voir [docs/status/CURRENT_STATE.md](../status/CURRENT_STATE.md). La section 3 de ce document (tableau de décisions V1) contenait encore une ligne obsolète affirmant le contraire ; corrigée par audit direct du code à l'occasion de la mission d'audit stacks (voir [INVENTORY_STACK_ARCHITECTURE.md](INVENTORY_STACK_ARCHITECTURE.md)).
- **World Containers V1 complet — toujours non terminé.** Manquent : interaction monde de production (`Component.IPressable`), prefab `Wooden_Crate`, UI de production, décisions de persistance, transfert partiel/stacking — conception de ce dernier point désormais produite, voir [INVENTORY_STACK_ARCHITECTURE.md](INVENTORY_STACK_ARCHITECTURE.md). Voir [../status/ROADMAP.md](../status/ROADMAP.md) pour l'ordre recommandé de la suite.

Ce document a été produit initialement par une mission de conception dédiée (branche `design/world-containers`, à partir de `main`/`df52f64`) — audit de l'existant puis architecture proposée. Le mécanisme de diffusion multi-viewer (section 7) a été confirmé par un spike technique isolé exécuté en runtime réel avec un host et deux clients distants (branche `spike/world-container-multiviewer-rpc`, voir [docs/research/WORLD_CONTAINER_MULTIVIEWER_SPIKE.md](../research/WORLD_CONTAINER_MULTIVIEWER_SPIKE.md)), puis le noyau lui-même a été implémenté et validé, puis les transferts d'item à leur tour implémentés et validés, comme décrit ci-dessus. Voir [../status/ROADMAP.md](../status/ROADMAP.md) et [../status/CURRENT_STATE.md](../status/CURRENT_STATE.md) pour l'état factuel du projet.
...
| Piles partielles ? | **Non à la date de cette section (V1 initiale).** Conception du transfert partiel désormais produite — voir [INVENTORY_STACK_ARCHITECTURE.md](INVENTORY_STACK_ARCHITECTURE.md), section 6.3 — non implémentée. | Cohérent avec la limitation déjà acceptée du drop (« pas de quantité choisie, pas de split ») — ne pas résoudre split/merge deux fois dans deux jalons différents. |
| Split/merge ? | **Non implémenté à ce jour.** Conception complète produite depuis — voir [INVENTORY_STACK_ARCHITECTURE.md](INVENTORY_STACK_ARCHITECTURE.md) (audit + politique d'identité + roadmap dédiée), pas encore construite. | Appartenait au futur jalon « Stacks, split/merge et quantités » de la roadmap (étape 8) au moment de la rédaction de cette section ; ce jalon a depuis sa propre conception dédiée. |
| Déplacement manuel dans la grille (drag & drop, position choisie) ? | **Obsolète — corrigé.** Était « Non » au moment de la rédaction de cette section ; **implémenté et validé par test réel depuis le 2026-07-21** (`RequestMoveItem`/`RequestStoreItemAt`/`RequestTakeItemAt`, campagne M1 host + client, y compris concurrence à deux fenêtres — voir [docs/status/CURRENT_STATE.md](../status/CURRENT_STATE.md)). | Voir la note en tête de document (section « Statut ») — cette ligne n'a pas été mise à jour au moment où le drag-and-drop a été livré ; corrigé ici par audit direct du code (`WorldContainerComponent.RequestMoveItem`/`RequestStoreItemAt`/`RequestTakeItemAt`, toutes trois présentes). |
...
**Écart réel constaté à l'implémentation (2026-07-19)** : l'alternative minimale envisagée ci-dessus (composer `TryAddFirstFit`+`TryRemove` tels quels, « ajouter avant de retirer ») n'a **pas** été retenue telle quelle. L'implémentation réelle ajoute une primitive supplémentaire, `InventoryContainer.TryFindFirstFit` (préflight pur, jamais mutant, partage sa sélection de candidat avec `TryAddFirstFit` via une méthode privée commune) pour obtenir une garantie strictement plus forte : **la source n'est jamais mutée avant que la destination ait confirmé pouvoir accepter l'item** — alors que la stratégie « ajouter avant de retirer » mute déjà la destination (source d'une éventuelle incohérence transitoire à gérer) avant de toucher la source. Le flux réel est : préflight destination → retrait source → ajout destination à la position déjà validée par le préflight (jamais un second scan). Un rollback exact reste prévu comme filet de sécurité si l'ajout planifié échoue malgré tout (anomalie, jamais le chemin normal) — voir [docs/research/WORLD_CONTAINER_TRANSFER_TESTS.md](../research/WORLD_CONTAINER_TRANSFER_TESTS.md) pour le détail et la validation runtime. Ceci ne modifie aucune des garanties déjà documentées ci-dessus (aucune duplication, aucune perte, même `InstanceId`) — seulement **comment** elles sont obtenues.
...
## 17. Matrice de tests proposée — Spike S0, matrice C0 et transferts whole-item, tous exécutés et validés
...
### Matrice T0-A à T0-O — **EXÉCUTÉE ET VALIDÉE EN RUNTIME RÉEL** (transferts whole-item)

Cette matrice remplace la matrice A à R proposée initialement dans cette section (conservée ci-dessous à titre de trace de la conception d'origine — la matrice réellement exécutée porte les identifiants T0-A à T0-O, voir [docs/research/WORLD_CONTAINER_TRANSFER_TESTS.md](../research/WORLD_CONTAINER_TRANSFER_TESTS.md) pour le rapport complet, les logs et les preuves scénario par scénario). Exécutée en runtime réel avec un host et jusqu'à deux clients distants, plusieurs sessions successives (branche `feature/world-container-transfers`). **15/15 scénarios PASS**, aucune anomalie critique (aucun `UnexpectedPlannedAddFailure`, aucun `CriticalRollbackFailure`, aucune duplication, aucune perte, aucun snapshot ou résultat mal ciblé).

Correspondance avec les scénarios de conception d'origine (colonne `#` de la matrice A à R ci-dessous) : T0-A≈C, T0-B≈D, T0-E≈E, T0-F≈F, T0-G/T0-H≈I (item absent), T0-I (identifiant invalide, non prévu explicitement dans la matrice d'origine), T0-J≈J/K, T0-K≈K, T0-L≈I (double requête), T0-M≈H (deux appelants), T0-N/T0-O (vérifications transverses destinataires/révisions, pas des scénarios ponctuels de la matrice d'origine). Le détail exact de chaque T0 (état initial, action, preuve, verdict) vit dans le rapport de test, pas ici — ce document ne fait que confirmer que la couverture visée par la matrice A à R est atteinte pour tout ce qui concerne les transferts.

**Non couverts par T0-A à T0-O**, restant hors périmètre de ce jalon (identifiants de la matrice A à R d'origine : M, N, O, P, Q, R) : late join avec conteneur déjà rempli, resynchronisation explicite pendant un transfert, indépendance de deux conteneurs distincts, cycles de transfert répétés, destruction du conteneur pendant la consultation — aucun n'est bloquant pour ce jalon, tous restent des extensions possibles de la matrice si un besoin réel apparaît.

### Matrice A à R — conception d'origine, conservée pour trace
...
**A/B/G/L déjà couverts par la matrice C0 (noyau, sans transfert) — voir section « Matrice C0-A à C0-J » ci-dessus.** C/D/E/F/H/I/J/K couverts par T0-A à T0-O (voir correspondance ci-dessus). M/N/O/P/Q/R restent non exécutés, non bloquants pour ce jalon.
...
6. ~~**Transfert conteneur → joueur.**~~ — **FAIT.** `RequestTakeItem`/`TryTakeItemAuthoritative` — voir [docs/research/WORLD_CONTAINER_TRANSFER_TESTS.md](../research/WORLD_CONTAINER_TRANSFER_TESTS.md).
7. ~~**Transfert joueur → conteneur.**~~ — **FAIT.** `RequestStoreItem`/`TryStoreItemAuthoritative`, même document.
8. ~~**UI debug minimale**~~ — **FAIT puis retiré après validation**, conformément à la politique déjà appliquée aux jalons précédents (`WorldContainerTransferDebugTools.cs`, `WorldContainerTransferDebugPanel.razor`/`.scss`, `WorldContainerTransferTest.scene`) — voir « Nettoyage » ci-dessous.
9. ~~**Robustesse multi-viewer**~~ — **FAIT.** Double requête, deux appelants distincts sur le même item (T0-L/T0-M), résultats et révisions confirmés par test réel — voir rapport de test.
10. ~~**Tests runtime T0-A à T0-O**~~ — **FAIT, 15/15 PASS.** Remplace et couvre la portée de la matrice A à R prévue pour les scénarios de transfert (C, D, E, F, H, I, J, K) — voir section 17 pour la correspondance exacte et les scénarios (M, N, O, P, Q, R) restant hors périmètre de ce jalon.
11. **Nettoyage et documentation** — retrait de tout outil de debug temporaire, mise à jour de ce document avec les résultats réels, plus `CURRENT_STATE.md`/`ROADMAP.md`/`OPEN_QUESTIONS.md`. **Fait pour les étapes 4/5** (ce document et les trois fichiers de statut mis à jour après la validation C0-A à C0-J) ; **fait pour les étapes 6/7/9/10** (cette mise à jour) — outils TEMP de transfert supprimés, ce document et les trois fichiers de statut mis à jour avec le résultat réel des transferts.

Les étapes 1 à 10 sont terminées et validées par test réel. L'étape 11 (nettoyage/documentation) est achevée pour ce jalon par la présente mise à jour. Restent après ce jalon : interaction monde de production (`Component.IPressable`), prefab `Wooden_Crate`, UI de production, persistance — voir [../status/ROADMAP.md](../status/ROADMAP.md).

### Nettoyage — outils TEMP de ce jalon, supprimés après validation

`Code/Debug/WorldContainerTransferDebugTools.cs`, `Code/UI/Debug/WorldContainerTransferDebugPanel.razor`/`.scss` et `Assets/scenes/Tests/WorldContainerTransferTest.scene` ont servi exclusivement à la validation runtime T0-A à T0-O (setup déterministe des deux côtés d'un transfert, déclenchement de Take/Store réels, déclenchement déterministe de deux Take concurrents depuis deux clients distincts pour T0-M) — supprimés intégralement après validation, conformément à la politique déjà appliquée aux outils de debug précédents (`PlayerVitalsDebugComponent`, spawner de loot, outils World Container Core). Aucune trace dans le code final — voir [docs/research/WORLD_CONTAINER_TRANSFER_TESTS.md](../research/WORLD_CONTAINER_TRANSFER_TESTS.md) pour le détail de ce qu'ils faisaient et les logs de session qu'ils ont produits.
