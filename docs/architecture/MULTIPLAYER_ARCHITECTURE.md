...
**Constat empirique complémentaire, confirmé sur Kodoku (`PlayerInventoryComponent`, test réel host + client, 2026-07-13)** : `IGameObjectNetworkEvents.StopControl()` peut se déclencher pour une instance qui n'a **jamais** eu le contrôle de l'objet — observé côté host au moment même du spawn du pawn d'un client distant, alors que le host n'en a jamais été le contrôleur. `StopControl` n'est donc pas garanti être précédé d'un `StartControl`/`OnStart` avec contrôle réel sur cette même instance ; tout code qui y réagit doit rester correct même appelé « à vide » dès la création de l'objet (voir [ITEM_ARCHITECTURE.md](ITEM_ARCHITECTURE.md), section « Snapshot d'inventaire propriétaire — V1 », pour le cas concret et sans conséquence observé).
...
**Question laissée ouverte, contournée par prudence (Kodoku, 2026-07-15)** : aucun précédent trouvé, ni dans le code moteur inspecté (`Sandbox.Engine.xml`) ni sur l'ancienne version du projet, confirmant qu'une propriété `[Sync]` définie sur un GameObject **avant** `NetworkSpawn()` soit incluse dans l'état initial transmis aux autres clients. Le seul ordre validé par test réel dans ce projet (Tests A à G, `LootSpawnPointComponent`, voir [ITEM_ARCHITECTURE.md](ITEM_ARCHITECTURE.md)) est `NetworkSpawn()` **puis** affectation des propriétés `[Sync]` — `PlayerItemDropComponent` (drop, voir ITEM_ARCHITECTURE.md) reproduit délibérément cet ordre plutôt que de tester l'inverse. **Cet ordre prudent (`Instance` locale posée avant le spawn, `[Sync]` publié après) a été validé par test réel host + client le 2026-07-15** : même `InstanceId` conservé sur huit cycles pickup/drop successifs, aucune nouvelle instance jamais créée par `OnStart()` sur un clone droppé — ce test confirme que l'ordre choisi fonctionne correctement, sans lever la question elle-même (l'ordre inverse, `[Sync]` avant spawn, reste non testé).
