...
- **Statut : conception auditée et corrigée, implémentation non commencée** — voir [PLAYER_INVENTORY_STACK_NETWORK_ARCHITECTURE.md](PLAYER_INVENTORY_STACK_NETWORK_ARCHITECTURE.md) pour l'audit dédié (corrigé par une relecture le 2026-07-24 : `AppliedRevision` ajouté au résultat réseau, stratégie `RollbackFailed` à trois cas, structure pending multi-items, formule exacte de la quantité de split) qui tranche les noms exacts de RPC/types laissés ouverts par la section 16 ci-dessous (`RequestSplitItem`/`RequestSplitItemAt`/`RequestMergeExact`/`RequestMergeUntilCapacity`, `PlayerInventoryStackResult`/`PlayerInventoryStackFailureReason`).
